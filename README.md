...
- `services/lawn-care.html`, `services/landscaping.html`, `services/irrigation.html`,
  `services/landscape-lighting.html`, `services/bush-hogging.html`, `services/fencing.html`,
...
- `.photo-placeholder` divs now mostly wrap real `<img>` tags. Two spots (`services/fencing.html` and
  `services/landscape-lighting.html`) still use a `.coming-soon` panel since no real photos exist yet for
  those services — swap those out once photography is available. Keep the `data-lightbox-label` /
  `data-full` attribute pattern on gallery page items since `js/main.js` depends on it for the lightbox.
...
- [ ] Replace `.coming-soon` panels on the fencing and landscape-lighting service pages with real photos
      once available
...
- Built the six service pages that were still missing (`landscaping`, `irrigation`, `landscape-lighting`,
  `bush-hogging`, `fencing`, `soft-washing-pressure-washing`)
